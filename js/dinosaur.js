...
const handleMessage = ({entities, traits}) => {
  ...
  if (celebrity) {
    ...
    console.log("How did dinosaurs extinct?'");
  } else {
    console.log("Human may extinct like dinosaurs.");
  }
};

const printWikidataDescription = (celebrity) => {
  const wikidataID = dinosaur.external && dinosaur.external.wikidata;
  if (!wikidataID) {
    // in case wikidata id isn't available
    console.log(`Dinosaur lived ${dinosaur.age} million years.`);
    return;
  }
